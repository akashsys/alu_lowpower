...
@cocotb.test()
async def test_lfsr_fairness_statistics(dut):
    await reset_dut(dut)
    dut.packet_size.value = 20
    grant_counts = [0, 0, 0, 0]
    sim_cycles = 500

    # Actively request on all ports for a long duration
    ...
    for _ in range(sim_cycles):
        ...
        if dut.grant_valid.value == 1:
            # Find which bit is set in the grant vector
            g_val = dut.grant.value.integer
            for i in range(4):
                if (g_val >> i) & 1:
                    grant_counts[i] += 1
    ...
    for i, count in enumerate(grant_counts):
        dut._log.info(f"Port {i} received {count} grants ({(count/sim_cycles)*100:.1f}%)")

    # Verification: Port 0 should win more, but Port 3 should NOT be zero.
    assert grant_counts[3] > 0, "FAIL: Port 3 was completely starved despite LFSR recovery!"
    dut._log.info("PASS: LFSR is successfully distributing bandwidth.")


@cocotb.test()
async def test_phantom_grant_check(dut):
    # Setup: Reset and provide enough credits
    await reset_dut(dut)
    dut.packet_size.value = 10
    
    # Cycle 1: Assert Request
    dut.request.value = 0b0001
    await RisingEdge(dut.clk)
    await Timer(1, units='ns') # Small delay to let combo logic settle
    
    # Check if we got a grant (In your current RTL, this takes 1 cycle)
    if dut.grant_valid.value == 1:
        dut._log.info("Grant received.")
        
        # Cycle 2: DROPPING REQUEST IMMEDIATELY
        dut.request.value = 0b0000
        
        # PROTOCOL CHECK: The grant MUST drop in the same cycle as the request
        # We use ReadOnly() to check the combinational state before the next clock
        await ReadOnly() 
        assert dut.grant_valid.value == 0, "FAIL: Phantom Grant detected! Grant stayed high after request dropped."
        dut._log.info("PASS: Zero-latency de-assertion verified.")